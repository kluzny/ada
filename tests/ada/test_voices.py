...
from ada.voices import Voices


@pytest.fixture
def mock_download_voice():
    """Mock the piper download_voice function."""
    with patch("ada.voices.download_voice") as mock:
        yield mock
...
def temp_voice_dir(tmp_path, monkeypatch):
    ...
    voice_dir.mkdir()
    monkeypatch.setattr(Voices, "CACHE_DIR", str(voice_dir))
    return voice_dir


def test_voices_initialization(temp_voice_dir, mock_download_voice):
    """Test Voices initialization with a voice identifier."""
    voice = Voices("en_US-amy-medium")

    assert voice.voice == "en_US-amy-medium"


def test_voices_downloads_when_missing(temp_voice_dir, mock_download_voice):
    """Test that Voices downloads the voice when files don't exist."""
...
def test_voices_skips_download_when_exists(temp_voice_dir, mock_download_voice):
    """Test that Voices skips download when voice files already exist."""
...
def test_voices_with_different_voice(temp_voice_dir, mock_download_voice):
    """Test Voices with a different voice identifier."""
    voice = Voices("en_GB-semaine-medium")
...
def test_voices_download_error_handling(temp_voice_dir):
    """Test that Voices raises error when download fails."""
    with patch("ada.voices.download_voice") as mock_download:
